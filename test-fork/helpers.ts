...
export const fullCycleLendingPool = async (
  ...
  const oToken = OToken__factory.connect(oTokenAddress, proposer);
  ...
  await (await oToken.connect(proposer).approve(pool.address, MAX_UINT_AMOUNT)).wait();
  ...
  expect(await oToken.balanceOf(proposer.address)).to.be.eq('0');
